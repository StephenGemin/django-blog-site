# scripts/add_post_script.py
# HOW TO RUN THIS
# python manage.py runscript add_post_script -v3
...
def run():
    ...
    # with open("post_generator.json") as f_json:
    #     posts = json.load(f_json)
    posts = post_list

    for post in posts:
        _p = Post(title=post["title"], content=post["content"], author_id=post["user_id"])
        _p.save()
